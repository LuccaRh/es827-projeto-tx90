...
Algumas das dificuldades que encontramos foram a obtenção do modelo do robô, para a qual foi necessário um esforço de pesquisa além do esperado até que fosse possível encontrá-lo. Outro ponto que se apresentou confuso no início foi a definição da trajetória: essa questão tomou algum tempo e algumas idas e vindas até que fosse possível montar a sequência de pontos que ficou como decisão final.

Outro grande ponto de dificuldade foi a validação reversa de todos os cálculos de cinemática direta e inversa. Uma vez que o grupo tomou a decisão de fazer essa verificação, um bom tempo e esforço dos membros foi dedicado a comprová-los manualmente. Ademais, a geração dos referenciais de torque e sua aplicação exigiu algumas tentativas até funcionar corretamente.
...
Gabriel Erick Matheus Imamura & Definição do modelo esquemático do braço robótico, da matriz DH e da cinemática direta e inversa, e parte da descrição da trajetória no relatório. \\ \hline
Matheus Gasques Siqueira & Definição dos pontos das trajetórias e parte da simulação. \\ \hline
Gabriel Zanette Cipriano & Revisão bibliográfica, contextualização da aplicação, pesquisa, introdução, conversão da trajetória, parte da simulação e resultados dinâmicos. \\ \hline
Lucca Ewertom Ryuichi Hirakawa & Revisão e organização do código e do relatório, e a parte de serpentina da trajetória. \\ \hline
João Victor Pomiglio de Oliveira & Definição do software de simulação, importação do modelo 3D, implementação da simulação, lógica de movimento suave e geração dos dados do modelo cinemáticos e dinâmicos. \\ \hline
\end{tabularx}
\end{table}

\subsection{Código MATLAB}

O código para gerar a trajetória, calcular a cinemática inversa, avaliar a dinâmica e simular os controladores é apresentado a seguir. 
Ele foi organizado em múltiplas pastas, com seus respectivos arquivos em cada uma. Elas foram feitas seguindo a convenção do MATLAB, em que o script principal define todos os parâmetros do projeto e orquestra as etapas, 
e cada etapa é implementada em uma função própria, sendo importada das pastas, que são usadas como bibliotecas. 
As funções foram agrupadas em quatro pacotes: \texttt{+robotica}, \texttt{+trajetoria}, \texttt{+controle} e \texttt{+visualizacao}, todos dentro da pasta funcoes.
O código em si foi feito seguindo as boas práticas do MATLAB, com comentários, nomes de variáveis e funções descritivos e em camelCase, e modularização das etapas.
\begingroup
\singlespacing
...
\lstinputlisting[caption={Geração da trajetória cartesiana com perfil LSPB, orquestrando contorno ou preenchimento (+trajetoria/gerarTrajetoria.m).},label={lst:gerarTrajetoria}]{\dirSimulacao/funcoes/+trajetoria/gerarTrajetoria.m}
\lstinputlisting[caption={Perfil temporal LSPB (+trajetoria/private/perfilLSPB.m).},label={lst:perfilLSPB}]{\dirSimulacao/funcoes/+trajetoria/private/perfilLSPB.m}
\lstinputlisting[caption={Interpolação LSPB entre waypoints em linha reta (+trajetoria/private/interpolarCartesianaConstante.m).},label={lst:interpolarCartesianaConstante}]{\dirSimulacao/funcoes/+trajetoria/private/interpolarCartesianaConstante.m}
\lstinputlisting[caption={Interpolação LSPB entre dois pontos, para os deslocamentos livres (+trajetoria/private/interpolarTransicao.m).},label={lst:interpolarTransicao}]{\dirSimulacao/funcoes/+trajetoria/private/interpolarTransicao.m}
\lstinputlisting[caption={Interpolação LSPB ao longo de um arco de círculo (+trajetoria/private/interpolarArco.m).},label={lst:interpolarArco}]{\dirSimulacao/funcoes/+trajetoria/private/interpolarArco.m}
\lstinputlisting[caption={Geração dos waypoints de preenchimento em serpentina (+trajetoria/private/rasterizarForma.m).},label={lst:rasterizarForma}]{\dirSimulacao/funcoes/+trajetoria/private/rasterizarForma.m}
\lstinputlisting[caption={Preenchimento da faixa branca por arcos concêntricos (+trajetoria/private/preencherFaixa.m).},label={lst:preencherFaixa}]{\dirSimulacao/funcoes/+trajetoria/private/preencherFaixa.m}
